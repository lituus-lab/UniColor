...
task clib, "C shared library":
  exec "nim c --app:lib -d:noAutoInit --noMain --mm:arc -d:release --path:src -o:" & sharedLib &
       macArgs & " src/UniColor/c_api.nim"
  done "clib"

task clibStatic, "C static library":
  exec "nim c --app:staticlib -d:noAutoInit --noMain --mm:arc -d:release --path:src -o:" &
...
task clibMsvc, "C static library, MSVC ABI (Windows Python extension)":
  ...
  exec "nim c --cc:vcc --app:staticlib -d:noAutoInit --noMain --mm:arc -d:release --path:src" &
...
task coverage, "LCOV + HTML coverage report for the Nim sources (needs lcov)":
  ...
  rmDir capiCache
  exec "nim c --app:staticlib -d:noAutoInit --noMain --mm:arc -d:release --path:src --debugger:native" &
       " --passC:--coverage --passL:--coverage --nimcache:" & capiCache &
       " -o:build/libUniColor_cov.a src/UniColor/c_api.nim"
  ...
  exec makeExe & " -C tests/c BIN=test_capi_cov" &
       " LIB=../../build/libUniColor_cov.a EXTRA_CFLAGS=--coverage"
  ...
  exec "lcov --capture --no-function-coverage --directory " & capiCache &
       " --base-directory ." &
       " --include \"*/src/UniColor/*\" --output-file build/capi.info --quiet" &
