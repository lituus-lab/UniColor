...
def nim_project_dir():
    """Where UniColor.nimble lives: the real repo root in a normal
...
def ensure_lib_built():
    ...
    try:
        ...
        raise SystemExit(
            "setup.py: `nimble` not found on PATH. Building unicolor from "
...
# The sdist ships the pre-transpiled unicolor/_core.c, not the .pyx (Cython
...
# needing no Cython.
pyx = os.path.join("unicolor", "_core.pyx")
ext = Extension(
    "unicolor._core",
    sources=[pyx if os.path.exists(os.path.join(HERE, pyx)) else os.path.join("unicolor", "_core.c")],
